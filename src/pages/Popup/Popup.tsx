...
const Popup = () => {
        ...
        chrome.tabs.sendMessage(
            ...
            async (proxyData) => {
                if (proxyData) {
                    ...
                    const oldProxyData = proxySettings.find(item => item.serverId === proxyData.serverId);
                    // 已存在该代理服务则更新为最新的
                    if (oldProxyData) {
                    ...
                    message.success('已更新 ' + proxyData.serverName+ ' 中的规则')
                } else {
    ...
    // 更新规则启用状态
    const updateRuleStatus = (serverId: number, ruleId: number, checked: boolean) => {
        if (!ruleId) return ;
    ...
    const refreshServer = (serverId: number) => {
        request(`/api/proxy-server/rule-list?proxy_server_id=${serverId}`).then(res => {
            if (res.success) {
                const rules = res.data?.data || [];
                // 只保留自己的规则
                const filterRules = rules.filter(rule => rule.ip === ip);
                const clone = cloneDeep(proxyServers);
                const server = clone.find(server => server.serverId === serverId);
                server.rules = filterRules;
                setProxyServers(clone);
                chrome.storage.local.set({proxySettings: proxyServers})
            ...
            <div className="content">
                {proxyServers.map((proxyServer) => (
                    <Card
                        ...
                        title={<div className='server-title'><span>{proxyServer.serverName}</span><ReloadOutlined onClick={() => refreshServer(proxyServer.serverId)}/></div>}
                        bordered={false}
                        style={{marginBottom: 18}}
                        headStyle={{height: 38, minHeight: 38, lineHeight: '38px'}}
                    >
                        {proxyServer.rules.map((rule) => (
                            <Row key={rule.id} justify='space-between' style={{marginBottom: 10}}>
                                <Col>{rule.remark}</Col>
                                <Col><Switch checked={rule.status === 1} onChange={(checked) => {updateRuleStatus(proxyServer.serverId,rule.id, checked)}} /></Col>
                            </Row>
                        ))}
                    </Card>
                ))}
